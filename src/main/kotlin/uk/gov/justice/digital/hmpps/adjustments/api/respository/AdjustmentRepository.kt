...
interface AdjustmentRepository : JpaRepository<Adjustment, UUID> {
  @Query("SELECT a FROM Adjustment a WHERE a.person = :person AND a.deleted = false " +
    "AND (a.fromDate IS NULL  OR a.fromDate >= :fromDate OR a.adjustmentType IN (:adjustmentTypes))")
  fun findCurrentAdjustmentsByPerson(person: String, fromDate: LocalDate, adjustmentTypes: List<AdjustmentType>? = listOf(REMAND, TAGGED_BAIL)): List<Adjustment>
}
